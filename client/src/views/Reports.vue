...
export default {
  ...
  setup() {
    ...
    const formatCurrency = (num) => {
      ...
      return Number(num).toLocaleString(locale, { style: 'currency', currency: currentCurrency.value })
    }
    ...
    const getChangeValue = (current, previous) => {
      ...
      const locale = currentCurrency.value === 'JPY' ? 'ja-JP' : 'en-US'
      const abs = Math.abs(change).toLocaleString(locale, { style: 'currency', currency: currentCurrency.value })
    ...
    return {
      ...
      t, formatCurrency, formatMonth, getBarHeight,
